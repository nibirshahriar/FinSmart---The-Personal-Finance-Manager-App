...
// Cloudinary config
const CLOUDINARY_CLOUD_NAME = "dplhz90b4";
const CLOUDINARY_UPLOAD_PRESET = "e2cbzdvv";
...
const Profile = () => {
  ...
  const uploadImage = async (uri) => {
    try {
      ...
      formData.append("public_id", `profilePics/${user.uid}`);
  ...
  const handleExportPDF = async () => {
    try {
      ...
      const rows = expenses
        ...
        .join("");
      const fileName = `FinSmart_Report_${displayName.replace(/\s+/g, "_")}_${new Date().toISOString().slice(0, 10)}.pdf`;
      const html = `
<html>
<head>
<style>
